...
public sealed class DagazRuneBehavior : RuneBehavior
{
    ...
    public override void UpdatePassive(RunePassiveContext context, RuneEntity rune, float deltaTime)
    {
        ...
        for (var i = 0; i < context.GameState.Runes.Count; i++)
        {
            ...
            if (ReferenceEquals(targetRune, rune) ||
                !targetRune.Presentation.IsCombatActive ||
                !DagazTuning.CanReceiveMultiShotBuff(targetRune.Stats.Type))
            {
                continue;
            }
            ...
            targetRune.Buffs.ApplyMultiShotDagazTier(rune.Stats.Tier);
        }
    }
}
